...
The setup script below is used to install the **EmcDementiaPredictionBase** package. You will need to modify this setup script as follows:
                                                                                              
- Set the `projectRootFolder` variable to the directory specific to your environment. In this example we are using `C:/EmcDementiaPredictionBase`. This root folder will serve a few purposes:
...
projectFolder <- "C:/EmcDementiaPredictionBase"
setwd(projectFolder)
                                                                                                 
# Download the lock file:
download.file("https://raw.githubusercontent.com/ohdsi-studies/EmcDementiaPredictionBase/master/renv.lock", "renv.lock")
...
You can safely continue by pressing 'y' after this prompt since the renv.lock file is downloaded from the **EmcDementiaPredictionBase** GitHub code repository. Once the installation is complete, you may need to restart R (if you are working outside of RStudio) and you should see this message:

````
Project 'C:/EmcDementiaPredictionBase' loaded. [renv 0.x.y]
````
